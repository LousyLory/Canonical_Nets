...
%%
load('arrdata_new_canning_erik.mat');
count = 0;
for i = 1: size(accuracy,1)
    maxacc = max(accuracy(i,1), accuracy(i,2));
    minacc = min(accuracy(i,1), accuracy(i,2));
...
plot(ax2, accuracy); title('validation accuracy of different models using canonicalization (with nomralization vide ELM) vs number of runs');
legend('model1 accuracy', 'model2 accuracy', '1+2 accuracy', '1+2(greedy) accuracy', '1+2(hun) accuracy');
xlabel('number of runs'), ylabel('validation accuracy');
descr={'Number of cases in which ';
    'accuracy of Hungarian lies';
...
plot(ax2, accuracy); title('validation accuracy of different models using full canonicalization (with nomralization vide ELM) vs number of runs');
legend('model1 accuracy', 'model2 accuracy', '1+2(original) accuracy', '1+2(normed) accuracy', '1+2(hun) accuracy', '1+2(greedy) accuracy');
xlabel('number of runs'), ylabel('validation accuracy');
descr={'Number of cases in which ';
    'accuracy of Hungarian lies';
    'within bounds: '; num2str(count)};
